        ...
        <Card class="h-full overflow-hidden rounded-none">
            ...
            >
                ...
                {#if gridCtx.editMode}
                    ...
                    >
                    <button
                        class="ml-2 flex size-5 shrink-0 cursor-pointer items-center justify-center rounded-sm text-muted-foreground transition-colors hover:bg-destructive/20 hover:text-destructive"
                        onclick={() => gridCtx.removeWidget(id ?? '')}
                    >
                        <XIcon class="size-3" />
                    </button>
                {/if}
